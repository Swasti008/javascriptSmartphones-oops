...
class IPhone extends SmartPhone {
  ...
  remainingAmount(priceIHave) {
    let actualPrice = this.price;
    this.price = priceIHave - this.price;
    return priceIHave < actualPrice? "Customer can't able to buy a phone due to insufficient amount": this.price <= 0 ? 'Customer has no more amount' : `Customer has remaining Rs.${this.price} after buying an iphone`;
  }
}
